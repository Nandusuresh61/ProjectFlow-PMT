...
export const TeamView = ({ openInvite }: any) => {
    const members = [
        ...
        <div className="space-y-10 max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-4 px-2">
                <div>
                    ...
                    <p className="text-[#576CBC]/60 font-medium">Manage team members and their roles</p>
                </div>
                <button
                    onClick={openInvite}
                    className="flex items-center gap-2 bg-black text-white px-6 py-3 rounded-xl font-bold text-sm border border-white/10 hover:bg-white/5 transition-all"
                >
                    <UserPlus size={18} />
            ...
            <div className="relative mb-8 max-w-md px-2">
                <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-[#576CBC]/40" size={18} />
                <input
                    type="text"
                    placeholder="Search members..."
                    className="w-full bg-white/[0.03] border border-white/5 rounded-xl py-3 pl-12 pr-4 text-sm text-white focus:outline-none focus:bg-white/[0.08] transition-all"
                />
            </div>

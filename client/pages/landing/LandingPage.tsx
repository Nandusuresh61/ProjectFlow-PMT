...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      isScrolled ? 'bg-black/60 backdrop-blur-xl border-b border-white/5 py-3' : 'bg-transparent border-b border-transparent py-5'
    }`}>
          ...
          >
            <div className="flex flex-col p-6 gap-6">
              {['Features', 'Solutions', 'Pricing', 'Login'].map((item) => (
                <a key={item} href="#" className="text-xl font-medium text-slate-300">{item}</a>
              ))}
              <button className="bg-white text-black py-4 rounded-xl font-bold">Get Started</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
const PricingCard = ({ tier, price, features, featured = false }) => (
  ...
  >
    <div className="mb-10">
      <h4 className={`text-xs font-black uppercase tracking-[0.2em] mb-4 ${featured ? 'text-black/50' : 'text-slate-500'}`}>{tier}</h4>
      <div className="flex items-baseline gap-1">
        <span className="text-6xl font-black tracking-tighter">${price}</span>
        <span className={featured ? 'text-black/40 text-sm' : 'text-slate-500 text-sm'}>/mo</span>
      </div>
    </div>
    ...
    <ul className="space-y-4 mb-10 flex-grow">
      {features.map((feature, i) => (
        <li key={i} className="flex items-start gap-3 text-sm font-medium">
          <CheckCircle2 size={18} className={featured ? 'text-black/20' : 'text-white/10'} />
          <span className={featured ? 'text-black/70' : 'text-slate-400'}>{feature}</span>
        </li>
      ))}
    </ul>
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black font-sans">
      ...
      <main className="relative">
        ...
        <section className="relative pt-48 pb-32 px-6">
          <div className="max-w-5xl mx-auto text-center">
            ...
            >
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Join the waitlist for ProjectFlow v2.0</span>
            </motion.div>
            ...
            >
              The most powerful project management tool for high-performance teams. 
            ...
            >
              ...
              {/* <motion.button 
                {...scaleHover}
                className="w-full sm:w-auto bg-transparent text-white border border-white/10 px-12 py-5 rounded-2xl text-lg font-bold hover:bg-white/5"
              >
                Watch Demo
              </motion.button> */}
            </motion.div>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-24 gap-8">
              <div className="max-w-2xl">
                <h2 className="text-xs font-black uppercase tracking-[0.3em] text-white/40 mb-4">Core Principles</h2>
                <h3 className="text-4xl md:text-6xl font-bold tracking-tight">Everything you need to <span className="text-white/40 italic">master the sprint.</span></h3>
              </div>
            </div>
        ...
        {/* --- PRICING --- */}
        <section id="pricing" className="py-32 px-6 bg-white/[0.02] border-y border-white/5">
          <div className="max-w-7xl mx-auto">
            <div className="text-center max-w-2xl mx-auto mb-24">
              <h2 className="text-xs font-black uppercase tracking-[0.3em] text-white/40 mb-4">Pricing</h2>
              <h3 className="text-4xl md:text-6xl font-bold tracking-tight mb-8">Ready to evolve?</h3>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
              <PricingCard 
                tier="Developer" 
                price="0" 
                features={["Up to 3 seats", "10 projects", "Real-time sync", "Community Support"]}
              />
              <PricingCard 
                tier="Enterprise" 
                price="19" 
                featured
                features={["Unlimited everything", "Priority Support", "SAML/SSO", "Dedicated Manager", "Custom SLAs"]}
              />
            </div>
          </div>
        </section>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-12 mb-20">
            <div>
              <div className="flex items-center gap-2 font-bold text-2xl tracking-tighter mb-6">
                <div className="bg-white text-black w-8 h-8 flex items-center justify-center rounded-lg">PF</div>
                ProjectFlow
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-16">
              <FooterList title="Product" links={['Features', 'Security', 'Roadmap']} />
              <FooterList title="Resource" links={['Guides', 'API', 'Docs']} />
              <FooterList title="Connect" links={['Twitter', 'Github', 'Discord']} />
            </div>
          </div>
          
          <div className="flex flex-col md:flex-row justify-between items-center pt-10 border-t border-white/5 gap-4">
             <p className="text-slate-600 text-[10px] font-bold uppercase tracking-widest">© 2025 ProjectFlow. Built for the future.</p>
...
function FooterList({ title, links }) {
  return (
    <div className="flex flex-col gap-4">
      <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30">{title}</h4>
